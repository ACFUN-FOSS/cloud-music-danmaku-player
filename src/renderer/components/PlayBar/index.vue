<template>
	<div class="player">
		...
		<div class="bar2">
			...
			<progress-bar :percent="percent" :bufferedPercent="bufferedPercent" :waiting="waiting"
				@percentChanged="onPercentChanged" @percentChanging="onPercentChanged"
				@virtualBarMove="onVirtualBarMove" @virtualBarLeave="onVirtualBarLeave" />
		...
		<div class="bar3">
			...
			<audio crossOrigin="anonymous" :id="source" ref="audio" @play="onPlay" @pause="onPause" @ended="onEnd"
				@timeupdate="updateTime" @waiting="onWaiting" @playing="onPlaying" @error="onError" />
		</div>
		<div class="bar4">
			...
			<span class="count-wrapper" @click="danmakuVisible=!danmakuVisible">
				<z-icon type="danmu" :isExternal="true" class="lrc" />
			</span>

			<z-icon type="geci" class="lrc" :class="{ active: isShowDesktoplyric }"
		...
		<a-drawer placement="right" wrapClassName="play-history" :width="550" :closable="false" :visible="visible"
			@close="closeDrawer">
			<div class="play-history-title" slot="title">
				<a-radio-group v-model="playComponent" buttonStyle="solid">
					...
					<a-radio-button value="current-play-table">点播列表</a-radio-button>
					<a-radio-button value="random-play-table">随机歌单</a-radio-button>
				</a-radio-group>
			</div>
			<component :is="playComponent"></component>
		</a-drawer>
		<a-drawer placement="right" wrapClassName="play-history" :width="550" :closable="false"
			:visible="danmakuVisible" @close="danmakuVisible = false">
			<div class="play-history-title" slot="title">
				弹幕点歌设置
			</div>
			<danmaku-panel />
...
import RandomPlayTable from "./RandomPlayTable";
...
export default {
	...
	methods: {
		...
		onEnd() {
			let { current_song_index, current_play_list } = this;
			this.currentTime = 0;
			this.buffered = 0;
			if (current_play_list[current_song_index]?.danmaku) {
				current_play_list.splice(current_song_index, 1);
			}
		...
		forward() {
			...
			if (!current_play_list.length) return;
			let newIndex = current_play_list.find((i) => i.danmaku);
			// 优先查找观众的歌曲
			if (newIndex === undefined) {
				newIndex = (current_song_index + 1) % current_play_list.length;
			}
			this.$store.commit("play/SET_CURRENT_INDEX", newIndex);
		},
		backward() {
			if (!this.isSongReady) {
				return;
