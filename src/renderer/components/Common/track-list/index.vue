<template>
	<div>
			...
			<div class="track-list" v-if="songs.length">
				<div v-if="isShowHead">
					...
					<div class="track-list-header" v-else>
						...
						<div v-for="col in currentColumns" :key="col.key" class="col-item" :style="
                                col.width
				...
				<div class="track-list-body">
					<ul class="song-list">
						<li v-for="(row, rowIndex) in songs" :key="rowIndex" :class="{ gray: row.matched === false }"
							...
							<a-dropdown :trigger="['contextmenu']" overlayClassName="sider-right-menu">
								<div class="song-item">
									...
									<div v-for="col in currentColumns" :key="col.key" class="col-item" :class="{
                                            'col-duration':
                                                col.key === 'duration',
                                        }" :style="
                                            col.width
                                        ...
                                        ">
										<div class="ellipsis">
											<slot :row="row" :column="col" :ke="col.key" :index="rowIndex"
												:name="col.slot" v-if="'slot' in col">
												<default-component :row="row" :col="col" />
											</slot>
											<component :is="
                                                    componentNames[col.key] ||
                                                    'DefaultComponent'
                                                " :row="row" :col="col" v-else>
											</component>
										</div>
									</div>
									...
									<div class="col-item col-actions" v-if="isShowActions">
										<song-heart :disable="row.songHeartDisable" :isLiked="
                                            ...
                                            " />
										<a-icon v-if="showRemove" type="close"
											@click.native.stop="removeSong(rowIndex)" />
									</div>
								</div>
								<a-menu slot="overlay">
									<a-menu-item key="0">
									...
									<a-menu-item key="7">
										<div @click="playTracks(rowIndex)" v-if="playTrack">
											<a-icon type="play-circle" />
											<span>播放专辑</span>
										</div>
									</a-menu-item>
		...
		<drap-modal centered title="新建歌单" okText="创建" :width="300" :maskClosable="false" v-model="visible"
...
export default {
	...
	methods: {
		...
		play(rowIndex) {
			this.$store.dispatch("play/appendPlay", this.songs[rowIndex]);
		},
		removeSong(idx) {
			this.$store.commit("play/REMOVE_SONG", idx);
		},
		nextPlay(song) {
